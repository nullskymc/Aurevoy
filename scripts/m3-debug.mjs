...
async function caseSymlinkDenied() { const _result = await runTask("M3_SYMLINK try symlink escape"); console.error("=== DEBUG traces ===", JSON.stringify(_result.traces.map(t=>({kind:t.kind,ok:t.ok,toolName:t.toolName,error:(t.errorMessage||"").slice(0,60)})),null,2)); console.error("=== END DEBUG ==="); return; async function _orig() {
