...
async function caseSymlinkDenied() {
  const _traces = await getTraces((await getJson("/api/tasks/" + (await postJson("/api/tasks", { goal: "M3_SYMLINK try symlink escape" })).task.id)).id);
  console.error("DEBUG traces:", JSON.stringify(_traces.traces.map(t => ({kind:t.kind, ok:t.ok, toolName:t.toolName, error:(t.errorMessage||"").slice(0,60)})), null, 2));
  return;
  async function caseSymlinkDenied() {
