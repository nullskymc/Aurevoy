...
export function buildToolGuidanceMessage(): Message {
  const content = [
    ...
    '',
    '**新增文件**：create_file 建空文件 → append_file 分次写入完整内容',
    '  - 大文件（> 200 行）优先用多次 append_file，每次写入 100-200 行',
    '  - append_file 是增量写入（不覆盖已有内容），适合分块构建文件',
    '',
    '**修改已有文件**：search_grep 定位行号 → replace_lines 精确替换',
    '  - 单次 replace_lines 建议不超过 200 行，超大内容会自动分块但写全量较慢',
    '',
    '**注意事项**：',
    '- 编辑前必须先用 search_grep 或 open_file 确认行号',
    '- 创建大文件时避免一次 replace_lines 写入全部内容',
