...
export function FileViewer({ tab, taskId, projectId }: FileViewerProps) {
      ...
      try {
        if (tab.kind === "workspace") {
          ...
          if (result.type === "image") {
            setState({ status: "image", content: result.content, mimeType: result.mimeType, path: result.path });
          } else if (result.type === "text") {
            setState({ status: "text", content: result.content, path: result.path, truncated: result.truncated });
          } else {
            setState({ status: "error", message: t("rightPanel.directoryPreviewUnsupported") });
          }
          return;
        }

        if (tab.kind !== "artifact") {
          setState({ status: "error", message: t("rightPanel.selectFile") });
          return;
        }
  ...
  if (state.status === "loading") {
    return <div className="file-viewer-state">{t("rightPanel.loading")}</div>;
  }
  ...
  if (state.status === "image") {
    return (
      <div className="file-viewer-image-wrap">
        <img className="file-viewer-image" src={`data:${state.mimeType};base64,${state.content}`} alt={state.path} />
      </div>
    );
  }
...
function TextPreview({ path, content, truncated }: { path: string; content: string; truncated: boolean }) {
  ...
  return (
    <div className="file-viewer-text">
      {truncated && <div className="file-viewer-truncated">{t("rightPanel.contentTruncated")}</div>}
      {isMarkdown ? (
        <div className="file-viewer-markdown">
          <MarkdownRenderer content={content} />
        </div>
      ) : (
        <pre className="file-viewer-pre"><code>{formatted}</code></pre>
      )}
    </div>
  );
}
